        ...
        <div class="item_list_container">
          ...
          <ul>
            <li class="item_list" v-for="(item, index) in itemDetail[itemNum -1 ].topic_answer" :key="index" @click="choosed(index, item.topic_answer_id)">
              <span class="option_style" :class="has_choosed(index, item.topic_answer_id)">{{chooseType(index)}}</span>
...
export default {
  ...
  methods: {
    ...
    has_choosed(index, id) {
      if(this.choosedNum === index && this.choosedId === id) {
        return 'has_choosed'
      }
    },
    nextItem() {
